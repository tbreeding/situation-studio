...
`deploy.sh` provides the RP1 versioned-release path and has been exercised successfully. It verifies locally, builds an immutable release, applies migrations and explicit web/service grants, imports the baseline idempotently, cuts over the `current` symlink, restarts the web, worker, publisher, live Leadership, and preview Leadership processes, and health-checks all origins. Credentials and Git authority are provisioned separately from immutable releases; gateway routes remain owner-UI operations.
