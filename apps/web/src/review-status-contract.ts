...
export const REVIEW_STATUS_SCHEMA_VERSION = "review-status-v4" as const;
export const REVIEW_STATUS_EVENT_NAME = "review-status" as const;
export const REVIEW_STAGE_TOTAL = 4 as const;
export const LEGACY_REVIEW_STAGE_TOTAL = 22 as const;
export const LEGACY_FOCUSED_REVIEW_STAGE_TOTAL = 24 as const;
const MAX_REVIEW_STAGE_TOTAL = LEGACY_FOCUSED_REVIEW_STAGE_TOTAL;
...
const publicStageSnapshotSchema = z.object({
  ordinal: z.number().int().min(1).max(MAX_REVIEW_STAGE_TOTAL),
...
const publicRetrySnapshotSchema = z.object({
  ...
  stageOrdinal: z.number().int().min(1).max(MAX_REVIEW_STAGE_TOTAL),
...
export const reviewStatusSnapshotSchema = z
  .object({
    ...
    completedStages: z.number().int().min(0).max(MAX_REVIEW_STAGE_TOTAL),
    totalStages: z.union([
      z.literal(REVIEW_STAGE_TOTAL),
      z.literal(LEGACY_REVIEW_STAGE_TOTAL),
      z.literal(LEGACY_FOCUSED_REVIEW_STAGE_TOTAL),
    ]),
    stages: z
      .array(publicStageSnapshotSchema)
      .min(REVIEW_STAGE_TOTAL)
      .max(MAX_REVIEW_STAGE_TOTAL),
