...
function isTransactionConflict(error: unknown) {
  ...
  return (
    ...
    /TransactionWriteConflict|could not serialize access/iu.test(
...
async function withTransactionRetry<T>(operation: () => Promise<T>) {
  ...
  throw new Error("Serializable transaction retries were exhausted.");
}
...
export async function renewCheckout(
  ...
  const renewed = await database.situationCheckout.updateMany({
    where: {
      id: input.checkoutId,
      releasedAt: null,
      holderUserId: input.userId,
  ...
  return withTransactionRetry(() =>
    database.$transaction(
      async (transaction) => {
        ...
        await transaction.$executeRaw`SELECT id FROM situations WHERE id = ${checkout.situationId}::uuid FOR UPDATE`;
        const released = await transaction.situationCheckout.updateMany({
          ...
          data: { releasedAt: now, releaseReason: "USER_CHECK_IN" },
        });
        if (released.count !== 1) return { ok: false as const };
    ...
    database.$transaction(
      async (transaction) => {
        ...
        await transaction.approval.updateMany({
          ...
          data: {
            invalidatedAt: now,
            invalidationReason: "MATERIAL_DRAFT_EDIT",
          },
        });
