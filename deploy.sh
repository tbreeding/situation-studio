...
echo "[1/9] Verifying the complete local workspace"
pnpm verify

echo "[2/9] Creating an immutable RP1 release"
...
echo "[3/9] Installing the pinned Node and pnpm toolchain"
...
echo "[4/9] Generating the ARM64 database client"
ssh "${studio_host}" "cd '${studio_release}' && source ~/.nvm/nvm.sh && nvm use && corepack pnpm db:generate"

echo "[5/9] Applying committed migrations with the migrator identity"
...
echo "[6/9] Granting explicit runtime privileges as the table owner"
...
echo "[7/9] Building with production cookie and origin policy"
...
echo "[8/9] Cutting over the release and reloading PM2"
...
echo "[9/9] Verifying liveness and database readiness"
